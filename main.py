...
def is_greeting(text: str) -> bool:
    ...
    prompt = f"""사용자의 입력이 단순한 인사인지 아닌지만 판단하세요.
    입력: {text}
    
    규칙:
    - 단순히 '0/1'로만 답변하세요
    - '안녕하세요', '안녕' 등의 인사말이면 '0'
    - 제품이나 다른 내용에 대한 질문이면 '1'
    """
    
    response = llm.invoke(prompt)
    return '0' in response.content 
...
def get_greeting_response(user_input: str) -> str:
    ...
    prompt = f"""사용자의 인사말에 대해 친절하고 자연스럽게 응답하세요.

    규칙:
    1. 사용자의 인사말에 간단한 인사로 응답
    2. 인사 후에는 문의 사항만 무엇인지 질문
    3. 친근하고 밝은 톤 사용
    4. 이모지 적절히 활용
...
def create_rag_chain(db: Chroma, product_uuid: str):
    ...
    def build_prompt(input_dict):
        ...
        4. 상품(구매,배송,이벤트 등) 관련 질문이 아니면 답변을 거부하세요
        상품 정보 및 과거 문의 내역:
        {context}
        """),
    ...
    try:
        ...
        if prompt := st.chat_input("무엇을 도와드릴까요?"):
            ...
            with st.chat_message("assistant"):
                # 첫 대화인 경우에만 인사 판별
                if len(st.session_state.messages) <= 1 and is_greeting(prompt):
